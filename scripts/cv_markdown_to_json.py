...
def parse_project(project_dir):
    """Parse project items from the _project directory."""
    project = []
    
    if not os.path.exists(project_dir):
        return project
    
    for project_file in sorted(glob.glob(os.path.join(project_dir, "*.md"))):
        ...
        if front_matter_match:
            ...
            project_entry = {
                "name": front_matter.get('title', ''),
                "category": front_matter.get('collection', 'project'),
                "date": front_matter.get('date', ''),
                "url": front_matter.get('permalink', ''),
                "description": front_matter.get('excerpt', '')
            }
            
            project.append(project_entry)
...
def create_cv_json(md_file, config_file, repo_root, output_file):
    ...
    # Add project
    cv_json["project"] = parse_project(os.path.join(repo_root, "_project"))
